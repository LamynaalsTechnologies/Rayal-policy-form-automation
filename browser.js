...
async function getDriver() {
  ...
  console.log(
    `[selenium] Candidate chromedrivers: ${candidateDrivers.join(", ")}`
  );
  ...
  try {
    ...
    const triedDriver = serviceBuilder
      ? "custom chromedriver (see log above)"
  ...
  await singletonDriver
    .manage()
    .setTimeouts({ implicit: 20000, pageLoad: 40000, script: 30000 });
...
async function openNewTab(driver, url) {
  ...
  while (attempts < maxAttempts && !newHandle) {
    ...
    try {
      ...
      if (newHandle) {
        ...
        console.warn(
          "[tabs] newWindow() did not create new tab, trying fallback"
        );
      }
    } catch (e) {
    ...
    try {
      console.log(
        "[tabs] Trying JavaScript window.open fallback with direct URL"
      );
      if (url) {
      ...
      const handlesAfter = await driver.getAllWindowHandles();
      console.log(
        `[tabs] Handles after JS open: ${handlesAfter.length}`,
        handlesAfter
      );
      ...
      if (newHandle) {
        console.log(
          "[tabs] Found new handle via JS, switching to it:",
          newHandle
        );
    ...
    if (!newHandle && attempts === maxAttempts) {
      ...
      try {
        ...
        console.error(
          "[tabs] Even new window creation failed:",
          (e && e.message) || e
        );
      }
    }
  ...
  if (!newHandle) {
    console.error(
      "[tabs] CRITICAL: Could not create new tab/window after all attempts"
    );
...
async function ensureLoggedIn(driver, waitMsIfNeeded = 20000) {
  ...
  const loginUrl =
    "https://www.uiic.in/GCWebPortal/login/LoginAction.do?p=login";
  const loginUrlPart = "LoginAction.do";

  // Helper: detect public site (non-portal)
  const onPublicUiicRoot = (url) =>
    (url.startsWith("https://uiic.co.in") ||
      url.startsWith("https://www.uiic.co.in")) &&
  ...
  if (currentUrl.includes(loginUrlPart)) {
    console.log(
      `Session not available. Please login manually. Waiting ${Math.floor(
        waitMsIfNeeded / 1000
      )} seconds...`
    );
  ...
  try {
    await driver.wait(
      until.elementLocated(By.xpath("//a[contains(@class, 'menuColor')]")),
      5000
    );
    console.log("Login verified.");
  } catch {
    console.log(
      "Could not verify login. You may still need to log in manually."
    );
  }
}
...
async function createFreshDriverFromBaseProfile(baseProfileDir) {
  console.log(
    "[driver] Creating fresh driver from base profile:",
    baseProfileDir
  );
  ...
  const tempProfileDir = path.join(
    __dirname,
    `chrome-profile-job-${timestamp}-${randomId}`
  );
  ...
  try {
    ...
    console.warn(
      "[driver] Failed to clone base profile, proceeding with empty profile:",
      (e && e.message) || e
    );
    try {
  ...
  try {
    ...
    console.error(
      "Failed creating driver with cloned profile:",
      (e && e.message) || e
    );
    throw e;
  }
...
/**
 * Get Chrome user data directory path (cross-platform)
 */
...
async function createChromeDriver(profileName = "InsurancePortalProfile") {
  // Define profile directory
  const baseChromePath = getUserDataPath();
  const timestamp = Date.now();
  const randomId = Math.random().toString(36).slice(2, 7);
  const tempProfilePath = path.join(
    os.tmpdir(),
    `chrome-profile-${profileName}-${timestamp}-${randomId}`
  );
  const profilePath = path.join(
    baseChromePath,
    `${profileName}-${timestamp}-${randomId}`
  );
